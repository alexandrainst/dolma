    ...
    def predict_slice(self, text_slice: TextSlice) -> Iterable[Prediction]:
        ...
        for label, score in zip(*pred):
            if label == "__label__da":
                return Prediction(label="da", score=score), Prediction(label="not_da", score=1.0 - score)
...
def add_global_language_score_from_slice_score_da(result: DocResult) -> DocResult:
    ...
    try:
        doc_da_score = sum((s.end - s.start) * s.score for s in result.spans if s.type == "da") / len(
            result.doc.text
        )
    ...
    doc_level = (
        ...
        Span(start=0, end=len(result.doc.text), type="doc_not_da", score=doc_not_da_score),
    )
...
class FastTextDanishLanguageParagraphWithDocScoreTagger(FastTextDanishLanguageParagraphTagger):
    def predict(self, doc: Document) -> DocResult:
        doc_result = super().predict(doc)
        doc_result = add_global_language_score_from_slice_score_da(doc_result)
        return doc_result
